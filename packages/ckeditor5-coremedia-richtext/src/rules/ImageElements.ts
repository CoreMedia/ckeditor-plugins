...
const imageElements: RuleConfig = {
...
const imageFigureElements: RuleConfig = {
  id: `transform-image-figure-element-attributes-bijective`,
  toData: {
    id: `toData-transform-image-figure-element-attributes`,

    imported: (node): Node => {
      if (isElement(node) && node.localName === "figure" && node.classList.contains("image")) {
        const result = renameElement(node, "p");
        result.classList.add("figure");
        return result;
      }
      return node;
    },
  ...
  toView: {
    id: `toView-transform-image-figure-element-attributes`,
    imported: (node): Node => {
      if (
        isElement(node) &&
        node.localName === "p" &&
        ...
        const result = renameElement(node, "figure");
        removeClass(node, "figure");
        return result;
      }
      return node;
    },
...
export const imageRules = [imageElements, imageFigureElements];
