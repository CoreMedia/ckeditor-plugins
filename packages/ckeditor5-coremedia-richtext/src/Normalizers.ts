...
/**
 * Normalizes HTML like collapsing white-space characters, sorting attributes
 * and sorting class-names in `class` attribute.
 *
 * Note, that we do not normalize CSS here, so that the transitive dependency
 * `clean-css` of `html-minifier-terser` we use inside here, is irrelevant.
 * Nevertheless, you may require to adapt, for example, your webpack
 * configuration.
 *
 ...
 * ```
 *
 * and apply these to the corresponding sections of your webpack configuration.
 */
export const normalizeHtml = async (value: string): Promise<string> => {
  return minify(value, {
